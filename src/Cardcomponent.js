...
export default function Cardcomponent() {
  ...
  for (let i = 0; i < questions.length; i++) {
    const link = links[i];
    cards.push(
      <Card key={i} align='center' h={250} variant='outline'>
        <CardBody>
          <Text margin={4}>{questions[i]}</Text>
        </CardBody>
        <CardFooter>
          <Button onClick={() => handleOpenModal(link)}>View code</Button>
        </CardFooter>
      </Card>
    );
  }
  ...
  return (
    <SimpleGrid columns={3} spacing={8} p={15}>
      {cards}
